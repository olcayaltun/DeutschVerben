...
// Debounce fonksiyonu (gereksiz localStorage yazmaları önlemek için)
...
const B2c1 = () => {
  ...
  // Kelime verilerini hazırla
  const words = Object.entries(wordsData);

  // Öğrenme ve tekrar dizisini oluşturma
  const generateSequence = useMemo(() => {
    return () => {
      if (!words.length) return [];

      const sequence = [];
      let wordIndex = 0;
      let learnedCount = 0; // 15 kelime sayacı

      while (wordIndex < words.length) {
        // Orijinal 3'lü grup mantığı
        const newWords = [wordIndex, wordIndex + 1, wordIndex + 2].filter(
          (i) => i < words.length
        );
        if (newWords.length) {
          sequence.push({ type: "learn", indices: newWords });
          learnedCount += newWords.length;
          wordIndex += newWords.length;
        }

        // Orijinal çapraz tekrar mantığı
        const period = Math.floor(sequence.length / 2) % 2;
        const startRepeatIndex = period === 0 ? 0 : 12;
        const repeatIndices = [startRepeatIndex, startRepeatIndex + 1].filter(
          (i) => i < words.length
        );
        if (repeatIndices.length) {
          sequence.push({ type: "repeat", indices: repeatIndices });
        }

        // Gruplar arası tekrar mantığı
        if (wordIndex < words.length) {
          const prevLast = wordIndex - 3;
          const currentFirst = wordIndex - 2;
          const repeatPair = [prevLast, currentFirst].filter(
            (i) => i >= 0 && i < words.length
          );
          if (repeatPair.length) {
            sequence.push({ type: "repeat", indices: repeatPair });
          }
        }

        // Yeni eklenen 15 kelime kuralı
        if (learnedCount >= 15) {
          const start = wordIndex - learnedCount;
          const end = start + 15;
          for (let i = start; i < end; i++) {
            if (i >= words.length) break;
            sequence.push({ type: "repeat", indices: [i] });
          }
          learnedCount = 0;
        }
      }

      // Kalan kelimeler için tek tekrar
      if (learnedCount > 0) {
        const start = wordIndex - learnedCount;
      ...
      // Orijinal son tekrar (ilk 2 kelime)
      if (words.length) {
        sequence.push({
          type: "repeat",
          indices: [0, 1].filter((i) => i < words.length),
        });
      }
  ...
  const validFavorites = favorites.filter((word) => wordsData[word]);

  // UI
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-gray-200 flex flex-col items-center justify-between p-4 sm:p-6">
      <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-2xl">
        ...
        {/* Ana Kart Alanı */}
        <div className="border border-gray-200 rounded-lg p-4 mb-6 bg-gray-50 shadow-inner">
          {words.length === 0 ? (
            <p className="text-gray-400 text-center">Kelime verisi yok...</p>
          ) : currentWords.length === 0 ? (
            <p className="text-gray-400 text-center">Kelime bulunamadı...</p>
          ) : (
            <div className="space-y-4">
              {currentWords.map((index) => (
                <div
                  key={index}
                  className="border-b border-gray-200 pb-3 last:border-b-0"
                >
                  <h2 className="text-xl font-semibold text-gray-900 mb-1.5">
                    {words[index]?.[0]?.replace(/_/g, " ") ||
                      "Kelime bulunamadı"}
                  </h2>
                  {showMeaning && (
                    <p className="text-gray-600 italic text-base">
                      {words[index]?.[1] || "Anlam bulunamadı"}
                    </p>
                  )}
                  <button
                    onClick={() => toggleFavorite(words[index]?.[0])}
                    className={`mt-2 w-full py-2 text-sm rounded-lg transition-all duration-200 font-medium ${
                      favorites.includes(words[index]?.[0])
          ...
          <button
            ...
            className="mt-4 w-full rounded-lg bg-blue-600 py-2.5 text-white font-medium hover:bg-blue-700 transition-colors"
          >
            {showMeaning ? "Anlamları Gizle" : "Anlamları Göster"}
          </button>
        </div>
        ...
        <div className="flex justify-between gap-3 mb-4">
          <button
            ...
            className="flex-1 rounded-lg bg-gray-600 py-2.5 px-4 text-white font-semibold hover:bg-gray-700 transition-colors"
          >
            ← Geri
          </button>
          <button
        ...
        <div className="mt-4">
          <button
            ...
            className="w-full rounded-lg bg-indigo-600 py-2.5 text-white font-semibold hover:bg-indigo-700 transition-colors"
          >
          ...
          {showFavorites && (
            <div className="mt-3 border border-gray-200 rounded-lg p-4 bg-gray-50">
              <h2 className="text-lg font-semibold text-gray-700 mb-3">
                Favori Kelimeler ({validFavorites.length})
              </h2>
